...
public class Block1 implements BlockInterface {
	
	private Image block = new ImageIcon(Main.class.getResource("../images/blocks/1.png")).getImage();
	...
	public void fixLocation() {
		...
		for(int i = 0; i < points.size(); i++) {
			...
			Board.position[points.get(i).getX()][points.get(i).getY()][1] = Board.COLOR_BLUE; // 각각 다름
		}
		try {
			Thread.sleep(200);
		...
		Board.newBlocks.remove(Block1.this);// 각각 다름.
				...
				if(isLine) {
					for(int j = i; j > 1; j--) {
						for(int k = 1; k <= 9; k++) {
							...
							Board.position[j][k][1] = Board.position[j - 1][k][1]; // 색깔..
						}
					}
